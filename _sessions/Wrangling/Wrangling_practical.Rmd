...
1. Open your `RBootcamp` R project. It should already have the folders `1_Data` and `2_Code`. Make sure that the data files listed in the `Datasets` section above are in your `1_Data` folder
...
5. If you haven't already, try putting all the code for your previous questions in one call to `mutate()`. That is, in one block of code, create `age_m`, `weight_lb`, `cd_change_20`, `cd_change_960` and `gender_char` using the `mutate()` function only once. Here's how your code should look:
